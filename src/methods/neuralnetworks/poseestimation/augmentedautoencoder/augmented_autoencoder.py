...
from src.methods.neuralnetworks.poseestimation.interface_pose_estimation import IPoseEstimation


class AugmentedAutoencoder(IPoseEstimation):
    def __init__(self, config_path, debug_vis, icp_flag=False):
        ...
        self.icp = icp_flag # test_args.getboolean('ICP','icp') #icp_flag
    ...
    def extract_square_patch(self, scene_img, bb_xywh, pad_factor,resize=(64,64),interpolation=cv2.INTER_NEAREST,black_borders=False):
    ...
    def process_detection(self, h, w, boxes, scores, labels):
    ...
    def pose_estimation(self, image, labels, boxes, masks, scores, depth_img=None, camPose=None):
        ...
        for j, (label, box_xywh, mask) in enumerate(zip(filtered_labels, filtered_boxes, masks)):
            ...
            #cv2.imshow("det img {}".format(j), det_img)
            ...
            all_cosine_similarity.append(cosine_similarity[0])

        end_time = time.time()
        # print("AAE Inference Time: {:.3f} s".format(end_time - start_time))
        return filtered_boxes, filtered_scores, filtered_labels, all_pose_estimates, all_class_idcs, all_cosine_similarity
    ...
    def draw(self, image, all_pose_estimates, all_class_idcs, labels, boxes, scores, cosine_similarities):
        ...
        for t, (label, box, score, cs) in enumerate(zip(labels, boxes, scores, cosine_similarities)):
            box = box.astype(np.int32)
            xmin, ymin, xmax, ymax = box[0], box[1], box[0]+box[2], box[1] + box[3]
            text = "{}: {:.3f}".format(label, cs)
            cv2.putText(
                im_show, text,
                (xmin, ymin - 5), cv2.FONT_ITALIC, .5, [30, 250, 0], 2 # self.color_dict[int(label)], 2
            )
            cv2.rectangle(im_show, (xmin, ymin), (xmax, ymax), (150, 150, 150), 1)
    ...
    def singles_renders(self, image, all_pose_estimates, all_class_idcs, labels, boxes, scores, cosine_similarities, alpha=1, beta=0.4):
        ...
        for i in range(len(all_pose_estimates)):
            ...
            renders_image = cv2.addWeighted(renders_image, alpha, g_y, beta, 0)
